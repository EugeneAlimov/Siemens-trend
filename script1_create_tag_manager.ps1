...
# Script to create TagManager class for manual tag management

# Project path (assuming script is launched from root project folder)
$projectPath = Get-Location

# Create directory for TagManager if it doesn't exist
$tagManagerDir = Join-Path -Path $projectPath -ChildPath "Storage\TagManagement"
if (-not (Test-Path $tagManagerDir)) {
    New-Item -Path $tagManagerDir -ItemType Directory -Force
}

# Create TagManager class
$tagManagerPath = Join-Path -Path $tagManagerDir -ChildPath "TagManager.cs"
$tagManagerContent = @"
using System;
...
"@
Set-Content -Path $tagManagerPath -Value $tagManagerContent

Write-Host "Script 1 completed successfully. TagManager class created."